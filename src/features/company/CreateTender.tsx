...
import { generateFullTender, generateTenderPDF, type GeminiProgress, type AIGeneratedTender, type PdfFormData } from "@/lib/gemini";
...
function CreateTender() {
  ...
  const [aiData, setAiData] = useState<AIGeneratedTender | null>(null);
  const [pdfFormSnap, setPdfFormSnap] = useState<PdfFormData | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const doc = await generateTenderPDF(pdfForm, ai);
  ...
  const handleDownload = () => {
    ...
    a.download = `Tender_${referenceNumber}.pdf`;
          ...
          <div className="modal" style={{ maxWidth: 980, width: "94%" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid #eee" }}>
              <div style={{ fontWeight: 600 }}>
                {referenceNumber} — Tender Preview
              </div>
              <button className="btn-outline" style={{ padding: 6 }} onClick={closePreview} aria-label="Close">
                <X size={16} />
              </button>
            </div>
            <div style={{ padding: 14 }}>
              <iframe
                src={previewUrl}
                title="Tender PDF Preview"
                style={{ width: "100%", height: "65vh", border: "1px solid #ddd", borderRadius: 6 }}
              />
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "space-between", padding: "12px 18px", borderTop: "1px solid #eee" }}>
