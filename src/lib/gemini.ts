...
const MODEL = PRIMARY_MODEL; // back-compat for buildTenderPrompt usage
const endpointFor = (model: string) =>
  `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${API_KEY}`;
const ENDPOINT = endpointFor(PRIMARY_MODEL);

function formatGeminiError(status: number, text: string) {
  let message = text;
  try {
  ...
  if (status === 404) return `Gemini model not available (${status}). ${message}`;
  if (status === 429) return `Gemini quota/rate limit reached. ${message}`;
  if (status === 503) return `Gemini service overloaded (503). ${message}`;
...
async function callGeminiModel(model: string, prompt: string): Promise<string> {
  ...
  if (!res.ok) {
    ...
    const err: any = new Error(formatGeminiError(res.status, txt));
...
async function callGeminiWithRetry(prompt: string, onProgress?: GeminiProgress): Promise<string> {
  ...
  for (const model of models) {
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        if (attempt > 1) onProgress?.(` Server busy, retrying (${attempt}/3) on ${model}...`);
        else onProgress?.(`✨ AI enhancing with ${model}...`);
        ...
        if (!retriable) break; // non-retriable -> try next model
        if (attempt < 3) await sleep(attempt * 2000);
      }
    }
    onProgress?.(" Switching to backup AI model...");
  }
...
export async function generateTenderPDF(formData: PdfFormData, ai: AIGeneratedTender) {
  ...
  sectionBar("1.  PROJECT DESCRIPTION");
  bodyPara(ai.projectDescription);

  sectionBar("2.  SCOPE OF WORK");
  numberedList(ai.scopeOfWork);

  sectionBar("3.  TECHNICAL SPECIFICATIONS");
  bodyPara(ai.technicalSpecifications);

  sectionBar("4.  TIME SCHEDULE OF TENDER ACTIVITIES");
  alphaList([
  ...
  sectionBar("5.  ELIGIBILITY CRITERIA");
  bodyPara(ai.qualificationSummary);
  y += 2;
  alphaList(ai.eligibilityCriteria);

  sectionBar("6.  REQUIRED SKILLS & EXPERTISE");
  skillPills(ai.requiredSkills);

  sectionBar("7.  FINANCIAL REQUIREMENTS");
  ...
  y += 5;
  bodyPara(ai.paymentTerms);

  sectionBar("8.  DOCUMENTS TO BE SUBMITTED BY VENDOR");
  numberedList(ai.requiredDocuments);

  sectionBar("9.  EVALUATION CRITERIA");
  bodyPara(ai.evaluationMethod);

  sectionBar("10.  GENERAL TERMS & CONDITIONS");
  numberedList(ai.generalConditions);

  sectionBar("11.  SUBMISSION INSTRUCTIONS");
  alphaList([
...
export async function generateVendorPDF(form: VendorPdfFormData, ai: VendorAIContent, match?: MatchEvaluation) {
  ...
  if (match) {
    sectionBar(`${form.uploadedDocs?.length ? "10" : "9"}.  AI EVALUATION SUMMARY`);
    infoRow("Match Score", `${match.totalScore} / 100`);
    infoRow("Verdict", match.verdict.replace("_", " "));
    infoRow("Price Fit", match.priceFit);
    bodyPara(match.summary);
  }

  sectionBar(`${match ? (form.uploadedDocs?.length ? "11" : "10") : (form.uploadedDocs?.length ? "10" : "9")}.  DECLARATION`);
